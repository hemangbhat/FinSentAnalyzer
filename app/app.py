...
try:
    from nlp_advanced import FinancialTextAnalyzer, analyze_financial_text
    from llm_enhanced import ChainOfThoughtReasoner, EliteExplanationGenerator
    ...
    with tab5:
        ...
        **Elite NLP Analysis** with chain-of-thought reasoning, financial lexicon scoring,
        ...
        if not ADVANCED_NLP_AVAILABLE:
            st.warning("Advanced NLP modules not available. Please ensure nlp_advanced.py and llm_enhanced.py are in the src folder.")
        else:
                    ...
                    with word_col1:
                        ...
                        if pos_words:
                            for word in pos_words[:8]:
                                st.markdown(f"<span style='background-color: #d4edda; padding: 3px 10px; border-radius: 15px; margin: 2px; display: inline-block;'>{word}</span>", unsafe_allow_html=True)
                        else:
                    ...
                    with word_col2:
                        ...
                        if neg_words:
                            for word in neg_words[:8]:
                                st.markdown(f"<span style='background-color: #f8d7da; padding: 3px 10px; border-radius: 15px; margin: 2px; display: inline-block;'>{word}</span>", unsafe_allow_html=True)
                        else:
                            st.markdown("*None detected*")
                    ...
                    with word_col3:
                        ...
                        if unc_words:
                            for word in unc_words[:8]:
                                st.markdown(f"<span style='background-color: #fff3cd; padding: 3px 10px; border-radius: 15px; margin: 2px; display: inline-block;'>{word}</span>", unsafe_allow_html=True)
                        else:
                            st.markdown("*None detected*")
