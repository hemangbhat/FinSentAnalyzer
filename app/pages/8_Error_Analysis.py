...
from utils import get_model_dir, LABEL_MAP_INV  # pyre-ignore
...
def get_error_analysis(model_name, data_split):
    ...
    results = pd.DataFrame({
        "sentence": X,
        "true_label": [LABEL_MAP_INV[int(l)] for l in y_true],
        "pred_label": [LABEL_MAP_INV[int(l)] for l in y_pred],
        "true_num": y_true,
        "pred_num": y_pred,
...
if len(errors_df) == 0:
    ...
    fig = go.Figure(go.Sankey(
        node=dict(
            ...
            label=["True: negative", "True: neutral", "True: positive",
    ...
    link_colors = {"negative": "rgba(239,68,68,0.3)", "neutral": "rgba(100,116,139,0.3)", "positive": "rgba(16,185,129,0.3)"}
