...
try:
    from integrate_news import (  # pyre-ignore
        ...
        load_news_headlines,
        predict_news_sentiment,
        compute_sentiment_stock_correlation,
    )
...
if not INTEGRATION_AVAILABLE:
    st.warning(
        "Cross-dataset integration module not available. "
        "Please ensure `integrate_news.py` is in the `src/` folder."
    )
    st.stop()
...
if cross_results_path.exists():
    ...
    if correlations:
        ...
        st.markdown("<p style='color: #94a3b8;'>Pearson correlation between daily news sentiment and stock returns</p>", unsafe_allow_html=True)
...
if st.button("📊 Generate Trend Chart", type="primary", use_container_width=True):
    try:
        ...
        if trend_data.empty:
            ...
            colors = [
                "#10b981" if v > 0 else "#ef4444" if v < 0 else "#64748b"
                for v in trend_data["mean_sentiment"]
            ]
            fig.add_trace(
                go.Bar(
                    x=trend_data["date"],
    ...
    for shap_file in shap_files:
        ...
        if top_features:
            with st.expander(f"🔍 {model_label} — Top SHAP Features", expanded=True):
                ...
                fig.update_layout(
                    title={"text": f"SHAP Feature Importance — {model_label}", "font": {"color": "#f8fafc", "size": 14}},
                ...
                if class_features:
                    ...
                    for i, (cls_name, cls_feats) in enumerate(class_features.items()):
                        with ccols[i]:
                            ...
                            st.markdown(f"<div style='color: {color}; font-weight: 600; text-transform: capitalize; margin-bottom: 10px;'>{cls_name} Class Drivers</div>", unsafe_allow_html=True)
                            for feat in cls_feats[:5]:
                                direction_icon = "↑" if feat["direction"] == "positive" else "↓"
                                st.markdown(f"<div style='display: flex; justify-content: space-between; padding: 3px 0; border-bottom: 1px solid rgba(255,255,255,0.05); font-size: 0.9em;'><span style='font-family: monospace; color: #cbd5e1;'>{feat['feature']}</span><span style='color: {color};'>{direction_icon} {feat['shap_value']:.3f}</span></div>", unsafe_allow_html=True)
else:
    st.info(
